...
def filter_long_samples(dataset):
    ...
    filtered_dataset = dataset.filter(lambda example: is_shorter_than_max_duration(example['audio']), num_proc=num_cores)
...
def filter_for_hebrew(dataset):
    ...
    filtered_dataset = dataset.filter(lambda example: contains_only_hebrew(example["transcription"]), num_proc=num_cores)
    return filtered_dataset
...
def get_vocab(dataset):
    ...
    for split, data in dataset.items():
        vocabs[split] = data.map(extract_all_chars, batched=True, batch_size=-1, keep_in_memory=True, remove_columns=data.column_names, num_proc=num_cores)
...
def standardize_dataset(dataset, dataset_name):
    ...
    dataset = dataset.remove_columns([col for col in dataset.features if col not in ["sentence", "transcription", "text", "audio"]])
    try:
    ...
    dataset = dataset.map(lambda x: {'dataset': dataset_name}, batch_size=-1, num_proc=num_cores)
    dataset = dataset.map(remove_special_characters, batch_size=-1, num_proc=num_cores)
    return dataset
...
def main():
    ...
    for dataset_config in DATASETS:
        ...
        split = None if TRAIN_AND_TEST else dataset_config['test_split']
        if LOAD_DATASET_FROM_LOCAL:
            dataset = load_from_disk(dataset_config['local_path'])
        else:
            dataset = load_dataset(path=path, name=dataset_config['language'], split=split)
